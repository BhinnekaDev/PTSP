const kirimEmailKunjunganAdmin = async (
  Email,
  Nama_Lengkap,
  ...
  const emailTujuan = {
    Meteorologi: "stamet.fatmawati@bmkg.go.id",
    Klimatologi: "staklim.bengkulu@bmkg.go.id",
    Geofisika: "stageof.kepahiang@bmkg.go.id",
  };
  const tujuanEmail = emailTujuan[dataPengajuan.Stasiun];
  ...
  <div style="font-family: Arial, sans-serif; max-width: 780px; margin: auto; border: 1px solid #ddd; border-radius: 8px; padding: 20px;">
    ...
    <div>
      ...
      <p style="font-size: 15px; color: #333; text-align: justify;">
        Telah diterima sebuah permohonan kunjungan dengan nomor <strong>${ID_PengajuanKunjungan}</strong> pada tanggal <strong>${Tanggal_Pengajuan_Kunjungan}</strong>.
      </p>
      ...
      <p><strong>Email Pengaju:</strong> <span style="color: #333;">
        ${Email}${Email_Perusahaan ? " - " + Email_Perusahaan : ""}
      </span></p>
...
`;
  ...
  try {
    await fetch("/api/postEmail", {
      ...
      body: JSON.stringify({
        to: tujuanEmail,
        subject: "Laporan Permintaan Kunjungan",
        html: htmlContent,
      }),
    });
  } catch (error) {
...
export default kirimEmailKunjunganAdmin;
